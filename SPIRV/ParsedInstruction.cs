	...
	public class ParsedInstruction
	{
		...
		private void ParseOperands()
		{
			...
			while (currentWord < Words.Count)
			{
				...
				operand.Type.ReadValue (Words.Skip(currentWord).ToList(), out object value, out int wordsUsed);
				...
				{
					ParsedOperand parsedOperand = new ParsedOperand(Words.Skip(currentWord).Take(wordsUsed).ToList(), value, operand);
			...
			if (varyingOperand != null)
			{
				...
				ParsedOperand parsedOperand = new ParsedOperand(Words.Skip(currentWord).ToList(), varOperantValue, varyingOperand);
				Operands.Add(parsedOperand);
			}
		}
